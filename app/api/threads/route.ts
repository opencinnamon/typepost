...
export async function GET(req: NextRequest) {
  const boardCode = req.nextUrl.searchParams.get("board");
  if (!boardCode) {
    return NextResponse.json({ error: "board query param required" }, { status: 400 });
  }
  const board = getBoardBySlug(boardCode);
  if (!board) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const subject = String(formData.get("subject") || "").slice(0, MAX_SUBJECT_LEN).trim();
    ...
    if (!body && (!imageFile || imageFile.size === 0)) {
      return NextResponse.json(
        { error: "Post must have a comment or an image." },
        { status: 400 }
      );
    }
    ...
    if (imageFile && imageFile.size > 0) {
      ...
      const path = `${board.slug}/${Date.now()}-${randomUUID()}.${ext}`;
